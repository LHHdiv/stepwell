...
这句话就是"一切皆插件"的兑现形式：你看到的每一件器官都是可替换的。第 43 讲毕业设计里，你会真的写一个 patch 把某个内置工具换成自己的实现——今天先学会"看片"。
...
从下一卷开始进入地基工程：dsh 敢于承诺"断电重启后一切如初"，靠的是一套仅追加的事件日志。第 05 讲我们先从一个看似不起眼却无处不在的类型技巧讲起——Branded ID，顺便开一门 TypeScript 类型体操小灶。
