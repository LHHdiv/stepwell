...
## 🔍 被否决的方案：删掉 bash 的 spill 文件？

Agent Note《drop-bash-output-spill-files》（rejected）：bash 工具为大体量输出维护私有临时 spill 文件，成本高昂——随机权限文件的创建、字节偏移的增量读取、有损读取报告、清理纪律。于是有提案干脆砍掉它。否决理由一句话：“完整输出恢复是真实的 bash 行为”——在替代产物服务就位前删它会丢失有用的命令输出。本章 spill 机制的存在不是过度设计，是被真实需求顶住的；连它的“删除提案”都留了后路：“未来的产物/blob 服务或许能将其泛化”。
